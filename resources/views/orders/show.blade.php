...
<div class="min-h-screen bg-[#FAFAF7] pb-20">
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 md:pt-10">
        ...
        {{-- ================================================================
             CARTES MARCHAND (Carte Gabon — MerchantCardPurchase)
             ================================================================ --}}
        @php $mp = $merchantPurchases ?? collect(); @endphp
        @if($mp->count() > 0)
            <section class="mb-10">
                ...
                <div class="space-y-4">
                    @foreach($mp as $i => $purchase)
                        ...
                        @endphp
                        <div x-data="{ codeShown: false, copied: false,
                                       copy(text) {
                             ...
                             class="bg-white rounded-2xl border border-slate-200 shadow-card overflow-hidden hover:shadow-card-hover transition-all">
                            <div class="grid grid-cols-1 md:grid-cols-[280px_1fr]">

                                {{-- Visuel marchand --}}
                                <div class="relative min-h-[180px]">
                                    @if($card && $card->visual_url)
                                        <div class="absolute inset-0 bg-cover bg-center" style="background-image: url('{{ asset($card->visual_url) }}');"></div>
                                    @else
                                        <div class="absolute inset-0 bg-gradient-to-br from-[#0F172A] to-[#0F4F44]"></div>
                                    @endif
                                    ...
                                    <div class="relative h-full p-5 flex flex-col justify-between text-white">
                                        <div class="flex items-center justify-between">
                                            <span class="inline-flex items-center gap-1.5 px-2 py-1 rounded-full bg-white/15 backdrop-blur-sm text-[10px] font-bold uppercase tracking-wider">
                                                <span class="w-1.5 h-1.5 rounded-sm bg-gradient-to-br from-[#4ECDC4] to-[#44A08D]"></span>
                                                KardAfrica
                                            </span>
                                            @if($isActive)
                                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/85 text-[10px] font-bold backdrop-blur-sm">
                                                    <span class="w-1.5 h-1.5 rounded-full bg-white"></span>
                                                    Active
                                                </span>
                                            @elseif($purchase->status === 'fully_used')
                                                <span class="px-2 py-0.5 rounded-full bg-slate-500/60 text-[10px] font-bold">Épuisée</span>
                                            @elseif($purchase->status === 'expired')
                                                <span class="px-2 py-0.5 rounded-full bg-rose-500/60 text-[10px] font-bold">Expirée</span>
                                            @endif
                                        </div>
                                        <div>
                                ...
                                <div class="p-5 flex flex-col gap-3">
                                    <div class="flex items-start justify-between gap-3 flex-wrap">
                                        <div>
                                            <div class="text-[10px] uppercase tracking-wider text-slate-400 font-bold">Solde restant</div>
                                            <div class="font-display text-2xl font-black text-slate-900 tabular-nums leading-none mt-1">
                                                {{ number_format($purchase->remaining_balance, 0, ',', ' ') }}
                                                <span class="text-xs font-bold text-slate-500">FCFA</span>
                                            </div>
                                            @if((float) $purchase->remaining_balance !== (float) $purchase->amount)
                                                <div class="text-[10px] text-slate-400 mt-0.5 tabular-nums">
                                                    sur {{ number_format($purchase->amount, 0, ',', ' ') }} achetés
                                                </div>
                                            @endif
                                        </div>
                                        <div class="text-right">
                                            <div class="text-[10px] uppercase tracking-wider text-slate-400 font-bold">Valable jusqu'au</div>
                                            <div class="text-sm font-bold text-slate-900 tabular-nums mt-1">{{ $purchase->expires_at->translatedFormat('d M Y') }}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="bg-slate-50 border border-slate-200 rounded-lg p-3">
                                        <div class="flex items-center justify-between mb-1.5">
                                            <span class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Code à présenter au marchand</span>
                                            <button @click="codeShown = !codeShown" type="button" class="text-[10px] font-bold text-[#44A08D] hover:underline">
                                                <span x-show="!codeShown">Afficher</span>
                                                <span x-show="codeShown" x-cloak>Masquer</span>
                                            </button>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <code x-show="!codeShown" class="flex-1 px-3 py-2 bg-white border border-slate-200 rounded-md font-mono text-base tracking-widest text-slate-400 select-none">•••• ••••</code>
                                            <code x-show="codeShown" x-cloak class="flex-1 px-3 py-2 bg-white border border-[#44A08D] rounded-md font-mono text-base tracking-widest font-bold text-slate-900 tabular-nums">{{ $purchase->unique_code }}</code>
                                            <button x-show="codeShown" x-cloak @click="copy('{{ $purchase->unique_code }}')" type="button"
                                                    class="shrink-0 px-3 py-2 rounded-md bg-slate-900 hover:bg-slate-700 text-white text-xs font-bold transition active:scale-95">
                                                <span x-show="!copied">Copier</span>
                                                <span x-show="copied" x-cloak>✓ Copié</span>
                                            </button>
                                        </div>
                                        <p class="text-[10px] text-slate-400 mt-2 leading-relaxed">
                                            Présente ce code ou ton QR code (à venir) au comptoir
                                            @if($merchant) chez <strong>{{ $merchant->business_name ?? $merchant->name }}</strong>@endif.
                                        </p>
                                    </div>
                                </div>
                            </div>
