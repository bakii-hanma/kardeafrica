...
<div style="padding:24px;font-family:'Inter','Figtree',sans-serif;"
    ...
    {{-- ============================================================
         API HEALTH BANNER
       ============================================================ --}}
    <div :style="`background:white;border-radius:16px;padding:18px 20px;margin-bottom:18px;
        ...
        <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;">
            <div :style="`width:46px;height:46px;border-radius:13px;display:flex;align-items:center;justify-content:center;flex-shrink:0;
                          background:${health.ok ? 'linear-gradient(135deg,#D1FAE5,#A7F3D0)' : 'linear-gradient(135deg,#FEE2E2,#FECACA)'};
            ...
            <div style="flex:1;min-width:240px;">
                <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.10em;color:#94A3B8;">API afrikard</div>
                ...
                <div style="font-size:13px;color:#475569;margin-top:3px;" x-text="health.message"></div>
            </div>

            <div style="display:flex;flex-direction:column;align-items:flex-end;font-size:11px;color:#64748B;line-height:1.5;">
                <div>
                    Latence : <span style="font-weight:700;color:#0F172A;font-variant-numeric:tabular-nums;" x-text="(health.latency_ms ?? 0) + ' ms'"></span>
                </div>
                <div x-show="health.checked_at">
                    Dernier check : <span style="font-weight:700;color:#0F172A;" x-text="(health.checked_at || '').slice(11,19)"></span>
                </div>
            </div>

            <button type="button" @click="testApi()" :disabled="loading"
                    style="display:inline-flex;align-items:center;gap:8px;padding:10px 18px;border:0;border-radius:11px;font-size:13px;font-weight:700;background:linear-gradient(135deg,#0F172A,#1E293B);color:white;box-shadow:0 8px 18px -8px rgba(15,23,42,0.4);"
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin-bottom:18px;">
        {{-- En attente --}}
        <div style="background:white;border-radius:14px;border:1px solid #E2E8F0;padding:18px 20px;display:flex;align-items:center;gap:14px;box-shadow:0 1px 2px rgba(15,23,42,0.04);">
            <div style="width:46px;height:46px;border-radius:13px;background:linear-gradient(135deg,#FEF3C7,#FDE68A);display:flex;align-items:center;justify-content:center;color:#B45309;flex-shrink:0;">
            ...
            <div style="min-width:0;">
                <div style="font-size:10px;text-transform:uppercase;letter-spacing:0.10em;font-weight:700;color:#94A3B8;">En attente de livraison</div>
                <div style="font-family:'Space Grotesk','Inter',sans-serif;font-size:24px;font-weight:800;color:#B45309;font-variant-numeric:tabular-nums;line-height:1.05;margin-top:3px;">
                    {{ $orders->total() }}
                </div>
            </div>
        </div>
        ...
        {{-- Sélectionnées --}}
        <div style="background:white;border-radius:14px;border:1px solid #E2E8F0;padding:18px 20px;display:flex;align-items:center;gap:14px;box-shadow:0 1px 2px rgba(15,23,42,0.04);">
            <div style="width:46px;height:46px;border-radius:13px;background:linear-gradient(135deg,#E0F2FE,#BAE6FD);display:flex;align-items:center;justify-content:center;color:#0369A1;flex-shrink:0;">
            ...
            <div style="min-width:0;">
                <div style="font-size:10px;text-transform:uppercase;letter-spacing:0.10em;font-weight:700;color:#94A3B8;">Sélectionnées</div>
                <div style="font-family:'Space Grotesk','Inter',sans-serif;font-size:24px;font-weight:800;color:#0369A1;font-variant-numeric:tabular-nums;line-height:1.05;margin-top:3px;"
                     x-text="selected.length"></div>
            </div>
        </div>

        {{-- Total à livrer --}}
        <div style="background:linear-gradient(135deg,#0F172A,#1E293B);border-radius:14px;padding:18px 20px;display:flex;align-items:center;gap:14px;box-shadow:0 4px 12px rgba(15,23,42,0.15);position:relative;overflow:hidden;">
            ...
            <div style="width:46px;height:46px;border-radius:13px;background:linear-gradient(135deg,#44A08D,#4ECDC4);display:flex;align-items:center;justify-content:center;color:white;flex-shrink:0;position:relative;">
            ...
            <div style="min-width:0;position:relative;">
                ...
                <div style="font-family:'Space Grotesk','Inter',sans-serif;font-size:22px;font-weight:800;color:white;font-variant-numeric:tabular-nums;line-height:1.05;margin-top:3px;">
    ...
    @if ($orders->count() > 0)
        <form method="POST" action="{{ route('admin.orders.retry-bulk') }}" data-no-loader
            ...
            {{-- Toolbar --}}
            <div style="background:white;border-radius:14px;padding:12px 16px;border:1px solid #E2E8F0;box-shadow:0 1px 2px rgba(15,23,42,0.04);margin-bottom:12px;display:flex;flex-wrap:wrap;align-items:center;gap:12px;">
                <label style="display:inline-flex;align-items:center;gap:10px;font-size:13px;font-weight:600;color:#475569;cursor:pointer;">
                    <input type="checkbox" @change="toggleAll($event.target.checked)" :checked="allSelected"
                           style="width:18px;height:18px;accent-color:#44A08D;">
                    Tout sélectionner sur cette page
                </label>

                <div style="margin-left:auto;display:flex;align-items:center;gap:14px;">
                    <span style="font-size:12px;color:#64748B;" x-show="selected.length > 0">
                        <span style="font-weight:700;color:#0F172A;" x-text="selected.length"></span> sélectionnée(s)
                    </span>
                    <button type="submit" :disabled="selected.length === 0 || !health.ok"
                            :style="`display:inline-flex;align-items:center;gap:8px;padding:11px 22px;border:0;border-radius:11px;font-size:13px;font-weight:700;color:white;
                                     ...
                                     box-shadow:0 10px 24px -10px rgba(68,160,141,0.5);
            ...
            <div style="display:flex;flex-direction:column;gap:8px;">
                @foreach ($orders as $order)
                    ...
                    <label :style="`background:white;border-radius:12px;padding:14px 16px;display:flex;align-items:center;gap:14px;flex-wrap:wrap;
                                    box-shadow:0 1px 2px rgba(15,23,42,0.04);transition:all .15s ease;cursor:pointer;
                                    border:${selected.includes({{ $order->id }}) ? '1px solid #44A08D' : '1px solid #E2E8F0'};
                                    ${selected.includes({{ $order->id }}) ? 'box-shadow:0 0 0 2px rgba(68,160,141,0.12);' : ''}`">

                        <input type="checkbox" :value="{{ $order->id }}" :checked="selected.includes({{ $order->id }})"
                               @change="toggleOne({{ $order->id }}, $event.target.checked)"
                               style="width:18px;height:18px;accent-color:#44A08D;flex-shrink:0;cursor:pointer;">

                        <div style="width:40px;height:40px;border-radius:11px;background:linear-gradient(135deg,#FEF3C7,#FDE68A);color:#B45309;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                        ...
                        <div style="display:flex;align-items:center;gap:6px;flex-shrink:0;flex-wrap:wrap;">
                            <span style="display:inline-flex;align-items:center;padding:3px 10px;border-radius:9999px;font-size:11px;font-weight:700;background:#D1FAE5;color:#047857;border:1px solid #A7F3D0;">
                                Payé
                            </span>
                            <span style="display:inline-flex;align-items:center;padding:3px 10px;border-radius:9999px;font-size:11px;font-weight:700;background:#FEF3C7;color:#B45309;border:1px solid #FDE68A;">
                        ...
                        <a href="{{ route('admin.orders.show', $order) }}"
                           @click.stop
                           style="width:32px;height:32px;border-radius:8px;background:#F8FAFC;border:1px solid #E2E8F0;display:flex;align-items:center;justify-content:center;color:#64748B;text-decoration:none;flex-shrink:0;"
    ...
    @else
        <div style="background:white;border-radius:14px;border:1px solid #E2E8F0;padding:60px 40px;text-align:center;box-shadow:0 1px 2px rgba(15,23,42,0.04);">
            <div style="width:64px;height:64px;border-radius:16px;background:linear-gradient(135deg,#D1FAE5,#A7F3D0);margin:0 auto 14px;display:flex;align-items:center;justify-content:center;color:#047857;">
            ...
            <div style="font-family:'Space Grotesk','Inter',sans-serif;font-size:18px;font-weight:700;color:#0F172A;margin-bottom:4px;">Aucune livraison en attente</div>
            <div style="font-size:13px;color:#64748B;">Toutes les commandes payées ont été livrées.</div>
        </div>
    @endif
</div>
