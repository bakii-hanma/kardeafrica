...
<div style="padding:24px;font-family:'Inter','Figtree',sans-serif;">
    ...
    @if(isset($filteredUser) && $filteredUser)
        <div style="background:linear-gradient(135deg,#ECFDF5,#D1FAE5);border:1px solid #6EE7B7;border-radius:14px;padding:14px 16px;margin-bottom:18px;display:flex;align-items:center;gap:14px;flex-wrap:wrap;">
            <div style="width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,#44A08D,#4ECDC4);color:white;display:flex;align-items:center;justify-content:center;font-family:'Space Grotesk','Inter',sans-serif;font-weight:800;font-size:16px;flex-shrink:0;">
                {{ strtoupper(substr($filteredUser->name, 0, 1)) }}
            </div>
            <div style="flex:1;min-width:200px;">
                <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.10em;color:#047857;">Filtré par client</div>
                <div style="font-family:'Space Grotesk','Inter',sans-serif;font-size:16px;font-weight:700;color:#0F172A;margin-top:2px;">{{ $filteredUser->name }}</div>
                <div style="font-size:12px;color:#475569;font-family:monospace;">{{ $filteredUser->email }}</div>
            </div>
            <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;">
                <div style="text-align:right;">
                    <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.10em;color:#94A3B8;">Cartes affichées</div>
                    <div style="font-family:'Space Grotesk','Inter',sans-serif;font-size:18px;font-weight:800;color:#0F172A;font-variant-numeric:tabular-nums;line-height:1.05;">{{ $cards->total() }}</div>
                </div>
                <a href="{{ route('admin.users.show', $filteredUser) }}"
                   style="display:inline-flex;align-items:center;gap:6px;padding:9px 14px;border-radius:10px;background:white;border:1px solid #6EE7B7;color:#047857;font-size:13px;font-weight:700;text-decoration:none;transition:all .15s;"
                ...
                <a href="{{ route('admin.cards.index') }}"
                   style="display:inline-flex;align-items:center;gap:6px;padding:9px 12px;border-radius:10px;background:rgba(15,23,42,0.05);color:#475569;font-size:12px;font-weight:600;text-decoration:none;"
    ...
    @if(isset($stuckOrders) && $stuckOrders->count() > 0)
        <div style="background:linear-gradient(135deg,#FEF3C7,#FDE68A);border:1px solid #FBBF24;border-radius:14px;padding:16px;margin-bottom:18px;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
                <div style="width:36px;height:36px;border-radius:10px;background:#F59E0B;color:white;display:flex;align-items:center;justify-content:center;">
                ...
                <div style="flex:1;">
                    ...
                    <div style="font-size:12px;color:#92400E;margin-top:2px;">L'API afrikard a échoué lors de ces paiements. Relancer la livraison ci-dessous.</div>
                </div>
            </div>
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:8px;">
                @foreach($stuckOrders as $stuckOrder)
                    <div style="background:white;border:1px solid #FCD34D;border-radius:10px;padding:10px;display:flex;align-items:center;gap:10px;">
                        <div style="flex:1;min-width:0;">
                            <div style="font-family:monospace;font-size:11px;font-weight:700;color:#0F172A;">#{{ $stuckOrder->order_number }}</div>
                        ...
                        <form action="{{ route('admin.orders.retry', $stuckOrder) }}" method="POST" style="margin:0;">
                            @csrf
                            <button type="submit"
                                    style="display:inline-flex;align-items:center;gap:4px;padding:6px 10px;background:#F59E0B;color:white;border:none;border-radius:7px;font-size:11px;font-weight:700;cursor:pointer;"
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(180px, 1fr));gap:14px;margin-bottom:18px;">
        @foreach ([
            ...
            <div style="background:white;border-radius:14px;padding:16px;border:1px solid #E2E8F0;box-shadow:0 1px 2px rgba(15,23,42,0.04);">
                ...
                <div style="font-family:'Space Grotesk', 'Inter', sans-serif;font-size:22px;font-weight:800;color:#0F172A;line-height:1.1;letter-spacing:-0.01em;">
                    {{ $card['value'] }}
                    @if(!empty($card['unit']))
                        <span style="font-size:11px;font-weight:500;color:#94A3B8;">{{ $card['unit'] }}</span>
                    @endif
                </div>
            </div>
        @endforeach
    ...
    <form method="GET" action="{{ route('admin.cards.index') }}"
        ...
        <input type="text" name="brand" value="{{ request('brand') }}" placeholder="Marque…"
               style="padding:10px 14px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:10px;font-size:13px;outline:none;">
        ...
        <div style="display:flex;gap:6px;">
            ...
            @if($activeFilters > 0)
                <a href="{{ route('admin.cards.index') }}" style="padding:10px 14px;color:#64748B;font-size:12px;font-weight:600;text-decoration:none;background:#F1F5F9;border-radius:10px;display:inline-flex;align-items:center;">✕</a>
            @endif
        </div>
    </form>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:16px;">
            @foreach($cards as $i => $card)
                ...
                <article x-data="{ codeShown: false, pinShown: false, copied: null,
                         ...
                         style="background:white;border-radius:16px;border:1px solid #E2E8F0;overflow:hidden;box-shadow:0 1px 2px rgba(15,23,42,0.04);transition:all 0.2s ease;"
                        ...
                        <div style="position:relative;z-index:1;display:flex;flex-direction:column;height:100%;justify-content:space-between;">
                            <div style="display:flex;align-items:flex-start;justify-content:space-between;">
                                <div style="min-width:0;flex:1;">
                                    ...
                                    <div style="font-family:'Space Grotesk', 'Inter', sans-serif;color:white;font-size:22px;font-weight:700;line-height:1.05;letter-spacing:-0.02em;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                                        {{ $card->brand ?: explode(' ', $card->name)[0] }}
                                    </div>
                                </div>
                                <div class="{{ $statusBadge['cls'] }}" style="padding:3px 8px;border-radius:9999px;font-size:10px;font-weight:700;color:white;display:inline-flex;align-items:center;gap:4px;backdrop-filter:blur(4px);margin-left:8px;flex-shrink:0;">
                                    @if($statusBadge['dot'])
                            ...
                            <div style="display:flex;align-items:flex-end;justify-content:space-between;">
                                <div>
                                    ...
                                    <div style="font-family:'Space Grotesk', 'Inter', sans-serif;color:white;font-size:18px;font-weight:700;font-variant-numeric:tabular-nums;margin-top:1px;">
                    ...
                    <div style="padding:14px;">
                        @if($card->user)
                            ...
                            <div style="margin-bottom:12px;padding-bottom:12px;border-bottom:1px solid #F1F5F9;">
                                ...
                                <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
                                    <div style="width:32px;height:32px;border-radius:9px;background:linear-gradient(135deg,#4ECDC4,#44A08D);display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:12px;flex-shrink:0;box-shadow:0 4px 8px -2px rgba(68,160,141,0.35);">
                                        {{ strtoupper(substr($card->user->name, 0, 1)) }}
                                    </div>
                                    <div style="flex:1;min-width:0;">
                                        <div style="font-size:13px;font-weight:700;color:#0F172A;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $card->user->name }}</div>
                                        <div style="font-size:10px;color:#64748B;font-family:monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $card->user->email }}</div>
                                    </div>
                                    @if($card->order)
                                        <a href="{{ route('admin.orders.show', $card->order_id) }}"
                                           style="font-family:monospace;font-size:10px;color:#44A08D;text-decoration:none;background:#ECFDF5;padding:4px 7px;border-radius:6px;font-weight:700;flex-shrink:0;border:1px solid #A7F3D0;"
                                ...
                                <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;">
                                    <a href="{{ route('admin.users.show', $card->user) }}"
                                       style="display:inline-flex;align-items:center;justify-content:center;gap:5px;padding:6px 10px;border-radius:8px;background:#F8FAFC;border:1px solid #E2E8F0;color:#475569;font-size:11px;font-weight:700;text-decoration:none;transition:all .15s;"
                                    ...
                                    @if($userCardsCount !== null)
                                        <a href="{{ route('admin.cards.index', ['user_id' => $card->user_id]) }}"
                                           style="display:inline-flex;align-items:center;justify-content:center;gap:5px;padding:6px 10px;border-radius:8px;background:rgba(68,160,141,0.10);border:1px solid rgba(68,160,141,0.30);color:#44A08D;font-size:11px;font-weight:700;text-decoration:none;transition:all .15s;"
                                    ...
                                    @else
                                        <span style="display:inline-flex;align-items:center;justify-content:center;gap:5px;padding:6px 10px;border-radius:8px;background:#F1F5F9;color:#94A3B8;font-size:11px;font-weight:700;">
                        ...
                        <div style="font-size:13px;font-weight:600;color:#0F172A;margin-bottom:10px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                            {{ $card->name }}
                        </div>
                        ...
                        <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:9px;padding:8px 10px;margin-bottom:8px;">
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
                                <span style="font-size:9px;text-transform:uppercase;letter-spacing:0.08em;font-weight:700;color:#94A3B8;">Code</span>
                                <div style="display:flex;align-items:center;gap:2px;">
                                    <button type="button" @click="codeShown = !codeShown" style="background:none;border:none;cursor:pointer;padding:3px;color:#94A3B8;border-radius:4px;">
                                    ...
                                    <button type="button" @click="copy('{{ $cardCode }}', 'code')" style="background:none;border:none;cursor:pointer;padding:3px;color:#94A3B8;border-radius:4px;">
                                        ...
                                        <svg x-show="copied === 'code'" x-cloak style="width:12px;height:12px;color:#059669;" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                                    </button>
                                </div>
                            </div>
                            <div style="font-family:monospace;font-size:11px;font-weight:700;color:#0F172A;letter-spacing:0.5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
                        ...
                        <div style="display:grid;grid-template-columns:{{ $cardPin ? '1fr 1fr' : '1fr' }};gap:6px;">
                            @if($cardPin)
                                <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:9px;padding:8px 10px;">
                                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
                                        <span style="font-size:9px;text-transform:uppercase;letter-spacing:0.08em;font-weight:700;color:#94A3B8;">PIN</span>
                                        <button type="button" @click="pinShown = !pinShown" style="background:none;border:none;cursor:pointer;padding:2px;color:#94A3B8;">
                                            <svg x-show="!pinShown" style="width:10px;height:10px;" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                            <svg x-show="pinShown" x-cloak style="width:10px;height:10px;" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.542 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>
                                        </button>
                                    </div>
                                    <div style="font-family:monospace;font-size:11px;font-weight:700;color:#0F172A;font-variant-numeric:tabular-nums;"
                                         x-text="pinShown ? '{{ $cardPin }}' : '{{ str_repeat('•', strlen($cardPin)) }}'"></div>
                                </div>
                            @endif
                            @if($card->expiration_date)
                                <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:9px;padding:8px 10px;">
                                    <div style="font-size:9px;text-transform:uppercase;letter-spacing:0.08em;font-weight:700;color:#94A3B8;margin-bottom:4px;">Expire</div>
                                    <div style="font-size:11px;font-weight:700;color:#0F172A;font-variant-numeric:tabular-nums;">
                        ...
                        <div style="display:flex;align-items:center;justify-content:space-between;margin-top:10px;padding-top:10px;border-top:1px solid #F1F5F9;">
                            <span style="font-size:10px;color:#94A3B8;">
                                ...
                                @if($card->serial_number)
                                    · <span style="font-family:monospace;color:#94A3B8;">{{ Str::limit($card->serial_number, 12, '…') }}</span>
                                @endif
                            </span>
                            <button type="button" @click="copy('{{ $cardCode }}', 'btn')"
                                    style="display:inline-flex;align-items:center;gap:4px;padding:5px 10px;background:#44A08D;color:white;border:none;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;">
    ...
    @else
        <div style="background:white;border-radius:14px;border:1px solid #E2E8F0;padding:60px 40px;text-align:center;box-shadow:0 1px 2px rgba(15,23,42,0.04);">
            ...
            <div style="font-family:'Space Grotesk', 'Inter', sans-serif;font-size:16px;font-weight:700;color:#0F172A;margin-bottom:4px;">Aucune carte trouvée</div>
            <div style="font-size:13px;color:#64748B;">{{ $activeFilters > 0 ? 'Essayez de modifier vos filtres' : 'Aucune carte n\'a encore été délivrée.' }}</div>
        </div>
    @endif
</div>
@endsection
